...
if command -v shellcheck &>/dev/null; then
    find "$PROJECT_DIR" -name "*.sh" -o -name "*.bash" \
        | grep -v tests/ \
        | xargs shellcheck --severity=style || { echo "  shellcheck found issues"; ((FAILED++)); }
    echo "  done"
else
...
if command -v hadolint &>/dev/null; then
    for df in Dockerfile.apt Dockerfile.brew Dockerfile.dev-debian_bookworm Dockerfile.dev-debian_trixie; do
        echo "  Linting $df ..."
        hadolint "$PROJECT_DIR/$df" || { echo "  hadolint failed on $df"; ((FAILED++)); }
    done
elif command -v docker &>/dev/null; then
    for df in Dockerfile.apt Dockerfile.brew Dockerfile.dev-debian_bookworm Dockerfile.dev-debian_trixie; do
        echo "  Linting $df (via docker) ..."
        docker run --rm -i hadolint/hadolint < "$PROJECT_DIR/$df" || { echo "  hadolint failed on $df"; ((FAILED++)); }
    done
else
...
if command -v bats &>/dev/null; then
    bats "$PROJECT_DIR/tests/bats/"*.bats || ((FAILED++))
else
...
if command -v python3 &>/dev/null && python3 -c "import pytest" &>/dev/null; then
    python3 -m pytest "$PROJECT_DIR/tests/python/" -v -m "not slow" || ((FAILED++))
else
